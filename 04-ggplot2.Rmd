...
p <- ggplot(surveys_complete, aes(x=weight, y=hindfoot_length)) + geom_points()
...
```{r limits}
xrange <- range(surveys_complete$weight)
yrange <- range(surveys_complete$hindfoot_length)

p <- surveys_complete %>% filter(species_id=="DM") %>%
...
```{r custom_colors}
p <- surveys_complete %>% filter(species_id %in% c("DM", "DS", "DO")) %>%
    ggplot(aes(x=weight, y=hindfoot_length)) +
